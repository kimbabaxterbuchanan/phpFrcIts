...
//Sanitize the POST values
    ...
    if ( $formAction != "delete") {
            ...
            if ( isset($companyId) && $companyId == "" )
                ...
                //Input Validations
            if($docNumber == '') {
                    $errmsg_arr[] = 'To RFQs Document Number missing';
                    $errflag = true;
                }
            if($title == '') {
                    $errmsg_arr[] = 'To RFQs title missing';
                    $errflag = true;
                }
            ...
            if ( !$errflag && $formAction != "view") {
                    $result = $toRFQsDAO->saveUpdateToRFQById ($id,$docNumber, $title,$customer, $value, $performancePeriod, $received_dt,
                        $questionDue_dt, $draftDue_dt, $finalDue_dt, $bid, $results, $notes, 
                        $tmName, $usrId, $posted, $companyId);
                        
                        //Check whether the query was successful or not
                    if($result) {
                            mkDirectory($homeDirectory.$awardDir."/toRFQs/".$docNumber."/");
                            if ( $section == "") {
                                    $section = "toRFQsInputs";
                                }
                            doUrl($hdrLocation, $paramString, $section, $sub_section);
            ...
            $result = $toRFQsDAO->deletetoRFQById ($id);
                //Check whether the query was successful or not
            if($result) {
                    rmDirectory($homeDirectory.$awardDir."/toRFQs/".$docNumber."/");
                    if ( $section == "") {
                            $section = "toRFQsInputs";
                        }
                    doUrl($hdrLocation, $paramString, $section, $sub_section);
...
//Create INSERT query
if ( $sub_section == "listToRFQs" && ! isset($formAction) ) {
    if ( $webAdmin == "yes") {
    ...
    $result = $toRFQsDAO->getToRFQById($id);
}
//Check whether the query was successful or not
if($errflag) {
